...
const PageVideo = () => {
   const bg = app.assets.video.hero.bg[0]
   const categories = Object.entries(sortObject(app.assets.video.categories))

   return (
      <>
         <Meta title={trans('pages.video.metaTitle')} description={trans('elements.video.metaDescription')} />
         <Hero assetType={app.assets.video} video={bg.file} bgPlaceholder={bg.placeholder} />
         <section className="mx-auto mb-10 px-4 lg:max-w-screen-xl">
            ...
            <p className="mb-8 text-center text-sm font-light text-gray-600">
               {trans(`elements.${app.assets.video.name}.descriptionAlt`)}
            </p>
                  ...
                  <li key={key}>
                     <Link
                        href={{
                           ...
                           query: {
                              assetType: app.assets.video.name,
