...
import { MusicSearchResultsAssetType } from '@lib/music/types'
...
type PropsType = {
   foundAssets: MusicSearchResultsAssetType[]
...
const MusicResults = ({ foundAssets, collections, handleSave, likedAssets, handleLike }: PropsType) => {
   ...
   const [active, setActive] = useState<MusicSearchResultsAssetType>()
   const [playing, setPlaying] = useState<string | undefined>()

   const onSaveClick = async (asset: MusicSearchResultsAssetType) => {
   ...
   const isSaved = (asset: MusicSearchResultsAssetType) => {
   ...
   const onLikeClick = async (asset: MusicSearchResultsAssetType) => {
   ...
   const isLiked = (asset: MusicSearchResultsAssetType) => {
      return likedAssets?.filter(likedAsset => {
         return likedAsset.assetUrl === asset.assetUrl
      }).length
   }

   const playPause = (asset: MusicSearchResultsAssetType = foundAssets[0]) => {
   ...
   return (
      <article className="xl:mx-auto xl:w-[70rem]">
            ...
            {foundAssets.map((asset, index) => (
               ...
               >
                  ...
                  <div className="ml-auto flex items-center gap-6 max-md:w-full">
                     ...
                     {asset.bpm && (
                        <div className="flex flex-col gap-1">
                           <span className="text-xs font-light text-gray-500">{trans('filters.tempo.bpm')}</span>
                           <span className="text-int font-semibold text-gray-700">{asset.bpm}</span>
                        </div>
                     )}
         ...
         {isFilled(foundAssets) && (
            <div
               ref={playerRef}
               className="fixed bottom-0 left-0 z-9 flex items-center justify-center gap-x-2 bg-slate-700 px-4 shadow max-md:right-0 max-md:flex-wrap max-md:py-4 md:bottom-2 md:left-1/2 md:h-16 md:-translate-x-1/2 md:gap-x-4 md:rounded-full"
            >
