import { musicTempoFilter } from '@components/search/music/filters/objects'
...
const { name, label, options } = musicTempoFilter

const TempoFilter = () => {
   const { query } = useRouter()
   const [current, setCurrent] = useState(query.tempo || null)
                     ...
                     return (
                        <span key={option.value} className="text-blue-600">
                           ...
                           <span className="text-xs text-gray-400">
                              {' '}
                              - {trans(`filters.tempo.${option.value}.bpm`)}
                           </span>
                        </span>
                     )
                  }
   ...
   return (
      <>
         ...
         <Listbox name={name} value={current} onChange={onOptionChange}>
            ...
            <Listbox.Options className="mt-1 grid grid-cols-1 rounded-md bg-gray-100 p-2">
               {options?.map(option => (
