import { videoFPSFilter } from '@components/search/video/filters/objects'
...
const { name, label, options } = videoFPSFilter

const FpsFilter = () => {
   const { query } = useRouter()
   const [current, setCurrent] = useState(query.fps ? query.fps.toString().split(',') : [])
   ...
   return (
      <>
         ...
         <Listbox value={current} onChange={onOptionsChange} multiple>
            ...
            <Listbox.Options className="mt-1 grid grid-cols-4 rounded-md bg-gray-100 p-2">
               {options?.map(option => (
