import { imagesAgeFilter } from '@components/search/images/filters/objects'
import { useAppContext } from '@contexts/app'
import { Listbox } from '@headlessui/react'
import IconCheck from '@icons/check.svg'
import IconChevronDown from '@icons/chevron-down.svg'
import { isTrue, trans } from '@lib/utils'
import { useRouter } from 'next/router'
import React, { Fragment, useEffect, useState } from 'react'

const { name, label, options } = imagesAgeFilter

const AgeFilter = () => {
   const { query } = useRouter()
   const AppContext = useAppContext()
   const [current, setCurrent] = useState(query.age ? query.age.toString().split(',') : [])

   const onOptionsChange = (selected: []) => {
      setCurrent(selected)

      AppContext.setSearchFilters({ age: selected.join(',') } as never)
   }
   ...
   return (
      <>
         ...
         <Listbox value={current} onChange={onOptionsChange} multiple>
            ...
            <Listbox.Options className="mt-1 grid grid-cols-3 rounded-md bg-gray-100 p-2">
               {options?.map(option => (
