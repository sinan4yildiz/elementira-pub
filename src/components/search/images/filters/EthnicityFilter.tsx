import { imagesEthnicityFilter } from '@components/search/images/filters/objects'
...
const { name, label, options } = imagesEthnicityFilter

const EthnicityFilter = () => {
   const { query } = useRouter()
   const AppContext = useAppContext()
   const [current, setCurrent] = useState(query.ethnicity ? query.ethnicity.toString().split(',') : [])

   const onOptionsChange = (selected: []) => {
      setCurrent(selected)

      AppContext.setSearchFilters({ ethnicity: selected.join(',') } as never)
   }
