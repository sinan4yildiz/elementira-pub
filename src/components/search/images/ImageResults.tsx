...
import { ImageSearchResultsAssetType } from '@lib/images/types'
...
type PropsType = {
   foundAssets: ImageSearchResultsAssetType[]
...
const ImageResults = ({ foundAssets, collections, handleSave, likedAssets, handleLike }: PropsType) => {
   ...
   const [assets, setAssets] = useState<ImageSearchResultsAssetType[][]>([])
   ...
   const onSaveClick = async (asset: ImageSearchResultsAssetType) => {
   ...
   const isSaved = (asset: ImageSearchResultsAssetType) => {
   ...
   const onLikeClick = async (asset: ImageSearchResultsAssetType) => {
   ...
   const isLiked = (asset: ImageSearchResultsAssetType) => {
                  ...
                  <li key={`${col}-${index}`}>
                     ...
                     >
                        ...
                        <a
                           ...
                           target="_blank"
                           rel="nofollow"
                           className="flex min-h-[10rem] items-center rounded-md ring-1 ring-inset ring-gray-200"
                        >
                           <Image
                              ...
                              className={`h-auto w-full ${
                                 ...
                                 asset.provider === 'CanStockPhoto' ||
                                 asset.provider === 'BigStockPhoto'
                                    ? 'scale-[115%]'
                                    : asset.provider === 'AdobeStock'
                                    ? 'scale-110'
                                    : ''
                              }`}
                           />
                        ...
                        {asset.contributor && (
                           <div className="pointer-events-none absolute left-1/2 top-12 z-5 mt-1 -translate-x-1/2 text-xs text-white opacity-0 group-hover:opacity-100">
                              {asset.contributor}
                           </div>
                        )}
